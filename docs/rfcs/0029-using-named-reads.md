# RFC 0029: `using`: a document names each read it takes from the request
...
`"using": { "agent": "@monitor/edge/request.resolvers.json#agent" }`, one entry per name, each a path to a resolvers
...
document makes is exactly its `using` map -- so the checker's walk from a trigger to the request paths it must
...
data graph or a binding **uses** a resolver by giving it a local name under `using`, bound to the document and the
...
```json
"using": { "agent": "@monitor/edge/request.resolvers.json#agent" },
```
...
```
P004  @features/monitor/data/create-row.graph.json#using/agent
...
```
P005  @features/monitor/data/create-row.graph.json#using/tenant
...
```
P006  @features/monitor/data/create-row.graph.json#using/saved
    'saved' is also the id of a node; {{saved}} would be ambiguous
    → rename the read: "using": { "savedBy": "...#saved" }
```
...
G003  @features/monitor/data/create-row.graph.json#nodes/saved/in/record/tenant
    ...
    → to read the request, bind the name: "using": { "tenant": "@monitor/edge/request.resolvers.json#tenant" }
```

Write `using` on a domain graph, and L002 refuses it as it refuses `resolvers` today: a domain graph never reads the
request.
...
**`graph.schema.json`** and **`binding.schema.json`**: `resolvers` (path) is removed; `using` (object, optional;
...
`resolvers` property's say "used under `using` as `@path#name`" where they say "names this document and reads
...
`using` and read as `{{name}}`"; the layers paragraph's sentence becomes "It is the only layer that reads the
request, and only through resolvers it names: `"using": { "agent": "@f/edge/request.resolvers.json#agent" }`, then
...
|---|---|---|---|
| P004 | `check/resolvers.ts`, `resolversFor`, at `using/<name>` | a `using` value is not `path#name` (the schema refuses most; this refuses what it cannot); its path names no resolvers document (R001, as today at `resolvers`); the document is another feature's and not visible (L005 through `judge.visible`, as today); or the document declares no resolver of that name | `wilanis describe <resolvers doc> lists its resolvers: <names>` |
| P005 | `check/graph.ts` after the nodes are judged; `check/bindings.ts` after the operations are | a `using` name that no value of the document reads (`Scope.templateReads` over every node's values, or every delegation's `in`, has no read rooted at it) | `read it as {{<name>}}, or drop the entry: using is exactly what this document reads` |
| P006 | `check/graph.ts`, at `using/<name>` | a `using` name is a node's id, or is in `RESERVED` (`in`, `const`, `request`, `secrets`) | `rename the read: "using": { "<name>By": "...#<resolver>" }` / `in, const, request and secrets are roots; pick another name` |
| G003 (existing) | `check/graph-reads.ts`, `rootReadRaw` | unchanged in what it refuses; the message names `using`: `'<root>' is not in, const, a node that runs before this one, or a name under using`, and the hint writes the entry | `to read the request, bind the name: "using": { "<root>": "@<feature>/edge/<file>.resolvers.json#<root>" }` |
| L002 (existing) | `check/resolvers.ts`, `resolversFor` | a domain graph has `using` (today: has `resolvers`); the `at` is `using` | unchanged |
| B-family (existing) | `check/bindings.ts`, `rootRead` | a delegation's value reads a root that is neither an input nor a `using` name; the message lists the `using` names where it lists the resolvers today | unchanged |

Everything that judged reads through resolvers judges them through `using`, with the same codes and the same
...
`request`, the `using` names, then nodes -- and P006 makes the order never matter.

### Runtime behaviour

Nothing runs differently. `lowerGraph` and the binding lowering in `packages/compiler/src/lower.ts` build
`roots.resolvers` (local name → the segments below `request`) from the `using` map and the judged resolvers, and
...
- `wilanis describe <graph>` and `<binding>` (`kindBody` in `packages/runtime/src/discovery.ts`) print a `using`
...
- `wilanis describe <resolvers document>` gains its case in `kindBody`: one line per resolver -- name, read,
  `required`, description -- and `used by <document> as {{<local>}}` for every graph or binding whose `using` names
...
- The viewer: `GraphView` in `packages/view/src/graphs.ts` builds its `resolvers` map from `using`, one entry per
...
None. A plugin's `check` that reads a graph's or a binding's reads (RFC 0015's X1n1 does) reads `using` where it
...
1.0 does (RFC 0008): `graph.schema.json` and `binding.schema.json` lose `resolvers` and gain `using`, so a document
written with `resolvers` no longer validates, and the schema's message says `using`. The four documents in this
...
RFC 0005's B0nn ("a binding of a required port names `resolvers`") and its sabotage case are amended to `using`;
RFC 0009's `publish-removal.graph.json` example is amended to `"using": { "token": ... }`. RFC 0015 is written in
...
|---|---|---|
| the schema | `packages/core/test/validate.test.ts` | the baseline data graph and binding carry `using`; a `resolvers` header is refused; a `using` value without `#`, with a name that is not an identifier, or an empty `using` is refused |
...
| G003 | sabotage | `{{agent}}` read with `using` removed: the message names `using` and the hint writes the entry |
| L002 | sabotage (existing case, edited) | `using` on `record-entry.graph.json` |
...
| `describe` | `packages/runtime/test/tools.test.ts` | the graph prints its `using` block; the resolvers document prints its resolvers and `used by` |
...
1. Core: `resolverRef` in `common.schema.json`; `using` replaces `resolvers` in `graph.schema.json`,
...
5. Viewer: the `resolvers` map from `using`, per-port `opens`. Test.
...
- **A reference in value position.** `{ "resolver": "@path#name" }` wherever a primitive is allowed was considered:
  ...
  document makes would again be a scan of the body. `using` gives the same openable path one line from the use,
...
- **`reads` instead of `using`.** `reads` is the tree's word for what a resolver is, and `"reads": { ... }` would say
  so. `using` says what the document does with them and reads well against `run`, `in`, `out`. The name is the
  maintainer's; this RFC uses `using` and nothing else depends on the word.
- **A shorthand `"using": "@doc"` for every name of a document** was considered and rejected: it is the implicit
...
1. The word: `using` or `reads`.
2. Whether P005 (an entry nothing reads) is a refusal, as written, or a note `describe` prints; a refusal keeps
   `using` exactly the read set, which is what makes the walk a map and what a small model can be held to.
