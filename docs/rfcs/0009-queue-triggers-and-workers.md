...
`{{token}}` is a resolver -- `request.resolvers.json` gains `token: { "read": "request.headers.authorization" }`
beside `agent` -- because the request is read in three places only, and a data graph is not one of them.
