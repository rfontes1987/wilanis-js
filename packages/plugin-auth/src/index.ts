...
type Env = Record<string, unknown> & {
  ...
  canon?: (r: string) => string;
  resolveType?: (r: string) => Type;
};
...
const sessionType = (env: Env): Type | undefined => {
...
function judged(attributes: Record<string, unknown>, env: Env): Record<string, unknown> {
  ...
  if (t) {
    const bad = conforms(attributes, t);
...
async function issueTokens(env: Env, session: SessionRecord): Promise<Record<string, unknown>> {
  const s = settingsOf(env),
    store = storeOf(env);
  const accessTtl = s.tokens?.accessTtl ?? 900,
    refreshTtl = s.tokens?.refreshTtl ?? 604800;
  ...
  const accessToken = await new SignJWT({ realm: session.realm, roles: session.roles, sid: session.id })
    ...
    .setIssuer(String(s.tokens?.issuer ?? ''))
    .setAudience(String(s.tokens?.audience ?? ''))
    .setIssuedAt()
...
const issue: Handler = async ({ in: i, ctx }) => {
  ...
  const session: SessionRecord = {
    ...
    realm: String(i.realm),
    roles: ((i.roles as string[]) ?? []).map(String),
    createdAt: iso(now()),
    attributes,
  };
...
const verify: Handler = async ({ in: i, ctx }) => {
  ...
  if (conn.kind === P('oidc.connection-kind.json'))
    return oidcVerify(
      conn.settings as {
        issuer: string;
        clientId: string;
...
async function oidcVerify(
  ...
  const withTimeout = async (url: string, init?: RequestInit) => {
    ...
    const t = setTimeout(() => ac.abort(), s.timeoutMs ?? 10000);
    try {
  ...
  const form = new URLSearchParams({
    ...
    username,
    password,
    scope: s.scope ?? 'openid profile',
    client_id: s.clientId,
    client_secret: s.clientSecret,
  });
  let res: Response;
  try {
    res = await withTimeout(disco.token_endpoint, {
      method: 'POST',
  ...
  try {
    claims = (
      await jwtVerify(body.id_token, createRemoteJWKSet(new URL(disco.jwks_uri)), {
        issuer: s.issuer,
        audience: s.clientId,
      })
    ).payload;
...
function place(read: unknown, value: string): string {
  ...
  switch (segs[1]) {
    case 'flags':
      return `--${segs[2]}=${value}`;
    case 'headers':
      return `header ${segs[2]}: ${value}`;
    case 'query':
      return `?${segs[2]}=${value}`;
    case 'cookies':
      return `cookie ${segs[2]}=${value}`;
    default:
...
const guard: Guard = {
  async identify({ credentials, settings, env }: GuardArgs) {
    ...
    if (credentials.token !== undefined) {
      ...
      let claims: JWTPayload;
      try {
        claims = (await jwtVerify(token, keyOf(s), { issuer: s.tokens?.issuer, audience: s.tokens?.audience })).payload;
      } catch (e) {
        return refuse(`the token does not verify: ${(e as Error).message}`);
      }
    ...
    if (answer && answer.id !== undefined && answer.id !== '') {
      ...
      if (record.subject && record.subject !== subject)
        return refuse(`challenge '${record.id}' was opened for another caller`);
  ...
  async challenge({ request, reads, settings, env, trigger, policy, message, method }) {
    ...
    const obtain = (s.challenge?.methods?.[record.method]?.obtain ?? 'obtain a code for challenge {id}').replaceAll(
      '{id}',
      id,
    );
...
function check({ scope, settings, refuse }: PluginCheckContext) {
  ...
  if (s.session && !shape)
    refuse({
      code: 'X101',
      file: '@project.json',
      message: `settings.session names '${s.session}', which is not a shape`,
      at: at('session'),
      hint: 'wilanis ls shape',
    });
  const methods = s.challenge?.methods ?? {};
  for (const p of scope.registry.all('policy'))
    for (const [reason, o] of Object.entries((p.doc as PolicyDoc).outcomes)) {
      if (o.effect === 'challenge' && o.method && !methods[o.method])
        refuse({
          code: 'X102',
          file: p.path,
          message: `outcome '${reason}' challenges by method '${o.method}', which ${ROOT} settings.challenge.methods does not declare`,
  ...
  for (const t of scope.registry.all('trigger')) {
    ...
    const challenges = uses.some(u =>
      Object.values(scope.get('policy', policyPath(u))?.doc.outcomes ?? {}).some(o => o.effect === 'challenge'),
    );
    if (challenges && !uses.some(u => typeof u !== 'string' && u.in?.challenge !== undefined))
      refuse({
        code: 'X102',
        file: t.path,
        message:
  ...
  if (shape) {
    ...
    const judge = (file: string, where: string, run: string, given: Record<string, unknown> | undefined) => {
      ...
      if (typeof type === 'string' && scope.canon(type) !== shape.path)
        refuse({
          code: 'X103',
          file,
          message: `${op} names type '${type}', but the session shape is '${s.session}'`,
          at: `${where}/type`,
          hint: `write "type": "${s.session}"`,
        });
      const values = given?.values;
      if (op === 'set' && values && typeof values === 'object' && !Array.isArray(values))
        for (const k of Object.keys(values))
          if (!fields.has(k) && !shape.doc.open)
            refuse({
              code: 'X103',
              file,
              message: `set writes '${k}', which ${s.session} does not declare`,
              at: `${where}/values/${k}`,
              hint: 'declare the attribute in the session shape, or drop it',
            });
      const keys = given?.keys;
      if (op === 'remove' && Array.isArray(keys))
        for (const k of keys)
          if (typeof k === 'string' && !fields.has(k) && !shape.doc.open)
            refuse({
              code: 'X103',
              file,
              message: `remove drops '${k}', which ${s.session} does not declare`,
              at: `${where}/keys`,
              hint: 'name attributes of the session shape',
            });
    };
    for (const g of scope.registry.all('graph'))
      for (const n of g.doc.nodes) if (isRun(n) || isMap(n)) judge(g.path, `nodes/${n.id}/in`, n.run, n.in);
    for (const b of scope.registry.all('binding'))
      for (const [name, op] of Object.entries(b.doc.operations))
        if (op.run) judge(b.path, `operations/${name}/in`, op.run, op.in);
  }
}
