...
import { type DocView, indexOf, type SchemaView, schemaRelOf, schemaViewOf, serveView, viewOf } from '../src/index.js';
...
describe('the view model of the example', () => {
  it('lists every document, sorted by kind then path, with no refusals', async () => {
    ...
    expect(idx.docs.map(d => d.kind)).toEqual(
      idx.docs
        .map(d => d.kind)
        .slice()
        .sort(),
  ...
  it('draws a data graph: in, every node, out, with typed ports', async () => {
    ...
    expect(asked.inputs.find(p => p.name === 'method')).toMatchObject({
    ...
    expect(asked.inputs.find(p => p.name === 'returns')).toMatchObject({
  ...
  it('draws a switch as a ladder: one rule node per rule, reading only what its condition names, then to its target, otherwise down or out', async () => {
    const v = await view(GET_ROW);
    const first = v.graph!.nodes.find(n => n.id === 'route/1')!,
      second = v.graph!.nodes.find(n => n.id === 'route/2')!;
  ...
  it('shows the out node as the fields it answers, fed by each candidate in order', async () => {
    ...
    expect(out.fields!.map(p => [p.name, p.type, p.required])).toEqual([
      ['id', 'string', true],
      ['url', 'string', true],
      ['method', 'string', true],
      ['ua', 'string', false],
  ...
  it('names who calls a graph: the binding that binds it, and the trigger that fires the port operation, via it', async () => {
    const v = await view(GET_ROW);
    expect(v.callers).toContainEqual({
  ...
  it('names the port operation a trigger fires and where it leads', async () => {
    ...
    expect(g.callers).toContainEqual({
      path: '@features/monitor/edge/get-entry.trigger.json',
      label: 'GET /monitor/{id}',
      kind: 'trigger',
      at: '/fire/run',
  ...
  it('says how a trigger answers each refusal it can reach, and which node refuses with it', async () => {
    ...
    expect(batch.answers?.find(a => a.reason === 'missing')).toMatchObject({
      answer: 404,
  ...
  it('views a policy: what it decides through, its outcomes, what it proves, and the triggers it gates', async () => {
    ...
    expect(v.decides).toMatchObject({
    ...
    expect(v.gates?.map(g => g.path)).toEqual(
      expect.arrayContaining([
    ...
    expect(t.answers!.find(a => a.reason === 'invalid_credential')).toMatchObject({
      answer: 401,
      from: [{ graph: '@auth/plugin.json', node: 'identify' }],
    });
    expect(t.answers!.find(a => a.reason === 'forbidden')?.from.map(f => f.graph)).toEqual(
      expect.arrayContaining([
  ...
  it('composes two nodes into one output: the digest', async () => {
    ...
    expect(v.graph!.nodes.find(n => n.id === 'digest')!.target).toMatchObject({
      portLabel: 'Objects',
      opName: 'make',
      pure: true,
    });
  ...
  it('a map with bind shows each bound input as read from the element, never as missing; an input typed by a bound variable shows the bound type', async () => {
    ...
    expect(recorded.inputs.map(p => [p.name, p.bound, p.missing])).toEqual([
  ...
  it('draws a graph that takes and answers a file: the blob type on the in and out nodes', async () => {
    ...
    expect(v.graph!.nodes.find(n => n.id === 'in')).toMatchObject({
      type: 'blob',
...
describe('the view server', () => {
  it('serves the page, the index and a document; refuses a missing path', async () => {
    ...
    try {
      ...
      const idx = (await (await fetch(`${s.url}api/index`)).json()) as {
        docs: unknown[];
        version: string;
        schemaBase: string;
      };
      ...
      const doc = (await (
        await fetch(`${s.url}api/doc?path=${encodeURIComponent('@monitor/data/get-row.graph.json')}`)
      ).json()) as DocView;
      ...
      const run = (await (
        await fetch(`${s.url}api/schema?path=${encodeURIComponent('node/run.schema.json')}`)
      ).json()) as SchemaView;
