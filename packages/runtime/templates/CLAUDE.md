...
|---|---|---|
| `project` | aliases, plugins (`use`, `from`, `settings` incl. the codecs table), secrets, startup, profiles | `project.json` |
...
- **Before the first request.** `project.json → startup` lists what runs once when `wilanis serve` starts, in order, before any trigger kind listens: open a pool, declare a queue, subscribe to a topic, check that storage answers. Each step names a **domain port operation** (`"run": "@tasks/domain/store.port.json#open"`), never a graph, so the profile's binding decides how it is met -- a fake in dev, the real connection in prod. A step's `in` is written as literals and `{{secrets.*}}` only: nothing has been received yet, so a step that reads `request.*`, or whose bound graph does, is refused (B007, B008). A step that refuses stops the server and exits nonzero, so a tree whose database is unreachable never opens its port; write `"required": false` for a step the tree can serve without, and its refusal is logged instead. This is the declared alternative to hiding boot work in a plugin's code: a reader sees at the root what happens before the first request.
