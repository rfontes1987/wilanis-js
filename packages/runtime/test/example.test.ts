...
import { loadTree, type PluginModule, type ResolvedInclude, schemaRef, schemaUrl } from '@wilanis/core';
...
describe('sabotage', () => {
  it('G003 a deep path that does not exist', () => {
    expect(
      sabotage('features/monitor/domain/record-entry.graph.json', d => {
        d.nodes[0].in.url = '{{in.urrl}}';
      }),
    ).toContain('G003');
  });
  it('G004 an optional read feeding a required input', () => {
    expect(
      sabotage('features/monitor/domain/list-entries.graph.json', d => {
        d.nodes[0].rules[0].when = 'true == true';
      }),
    ).toContain('G004');
  });
  ...
  it('G008 a node nobody reads', () => {
    expect(
      sabotage('features/monitor/domain/digest.graph.json', d => {
        d.out.from = 'all';
        d.out.type = '@monitor/domain/Entry.shape.json[]';
      }),
    ).toContain('G008');
  });
  ...
  it('L002 an effect run from a domain graph', () => {
    expect(
      sabotage('features/monitor/domain/digest.graph.json', d => {
        d.nodes[0].run = '@http/http.port.json#request';
        d.nodes[0].in = { connection: '@connections/monitor-api.connection.json', method: 'GET', path: '/x' };
      }),
    ).toContain('L002');
  });
  ...
  it('D008 a shape whose declared layer contradicts the directory it sits in', () => {
    expect(
      sabotage('features/monitor/domain/Entry.shape.json', d => {
        d.layer = 'edge';
      }),
    ).toContain('D008');
  });
  it('L006 a trigger that fires a native operation instead of a domain port', () => {
    expect(
      sabotage('features/monitor/edge/digest.trigger.json', d => {
        d.fire.run = '@std/list.port.json#count';
      }),
    ).toContain('L006');
  });
  ...
  it('L003 an effect the feature does not allow', () => {
    expect(
      sabotage('features/monitor/feature.json', d => {
        d.effects = [];
      }),
    ).toContain('L003');
  });
  it('T002 a trigger whose edge shape does not fit the graph', () => {
    expect(
      sabotage('features/monitor/edge/RecordRequest.shape.json', d => {
        delete d.fields.url;
      }),
    ).toContain('T002');
  });
  it('T004 a resolver reading request.* under a kind that hands none', () => {
    ...
    expect(
      sabotage('features/monitor/data/list-rows.graph.json', d => {
  ...
  it('R001 a resolvers document that does not exist', () => {
    expect(
      sabotage('features/monitor/data/create-row.graph.json', d => {
        d.resolvers = '@monitor/edge/nope.resolvers.json';
      }),
    ).toContain('R001');
  });
  it('G003 a read of a resolver the named document does not define', () => {
    expect(
      sabotage('features/monitor/data/create-row.graph.json', d => {
        d.nodes[0].in.headers = { 'x-forwarded-user-agent': '{{caller}}' };
      }),
    ).toContain('G003');
  });
  it('P002 a resolver reading a path no trigger kind hands', () => {
    expect(
      sabotage('features/monitor/edge/request.resolvers.json', d => {
        d.resolvers.agent.read = 'request.nowhere.session';
      }),
    ).toContain('P002');
  });
  it('P003 a resolver named like a root', () => {
    expect(
      sabotage('features/monitor/edge/request.resolvers.json', d => {
        d.resolvers.in = { read: 'request.headers.host' };
      }),
    ).toContain('P003');
  });
  it('D001 a resolver whose read does not start at the request', () => {
    expect(
      sabotage('features/monitor/edge/request.resolvers.json', d => {
        d.resolvers.agent.read = "headers['user-agent']";
      }),
    ).toContain('D001');
  });
  ...
  it('T005 a refusal reason the trigger can reach but does not map', () => {
    expect(
      sabotage('features/monitor/edge/get-entry.trigger.json', t => {
        delete t.settings.response.refusals.missing;
      }),
    ).toEqual(['T005']);
  });
  it('T005 a reason reached only through a map, from the batch delete', () => {
    expect(
      sabotage('features/monitor/edge/delete-entries.trigger.json', t => {
        delete t.settings.response.refusals.missing;
      }),
    ).toEqual(['T005']);
  });
  it('T006 a mapped reason nothing the trigger fires refuses with', () => {
    expect(
      sabotage('features/monitor/edge/get-entry.trigger.json', t => {
        t.settings.response.refusals.teapot = 418;
      }),
    ).toEqual(['T006']);
  });
  it('P001 a reason given as a read: the checker must see the word', () => {
    ...
    expect(
      sabotage('features/monitor/data/get-row.graph.json', g => {
        g.nodes.find((n: any) => n.id === 'missing').in.reason = '{{asked.status}}';
      }).sort(),
    ).toEqual(['P001', 'T006']);
  });
  it('G005 a refusal without a reason', () => {
    expect(
      sabotage('features/monitor/data/get-row.graph.json', g => {
        delete g.nodes.find((n: any) => n.id === 'missing').in.reason;
      }).sort(),
    ).toEqual(['G005', 'T006']);
  });
  it('G006 an input the operation does not declare', () => {
    expect(
      sabotage('features/monitor/data/list-rows.graph.json', d => {
        d.nodes[0].in.query = { a: 'b' };
      }),
    ).toContain('G006');
  });
  it('P001 a static field given a read', () => {
    expect(
      sabotage('features/monitor/data/list-rows-by-method.graph.json', d => {
        d.nodes[0].in.method = '{{in.method}}';
      }),
    ).toContain('P001');
  });
  it('G003 a read of a node that does not exist', () => {
    expect(
      sabotage('features/monitor/data/list-rows.graph.json', d => {
        d.nodes[2].in.value = '{{asked2.body}}';
      }),
    ).toContain('G003');
  });
  it('T003 a route placeholder the route does not declare', () => {
    expect(
      sabotage('features/monitor/edge/get-entry.trigger.json', d => {
        d.settings.route = '/monitor/{entry}';
      }),
    ).toContain('T003');
  });
