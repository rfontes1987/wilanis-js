...
export class FileBlobStore implements BlobStore {
  ...
  constructor(root: string, dir?: string) {
    this.dir = dir ? (isAbsolute(dir) ? dir : resolve(root, dir)) : join(tmpdir(), `wilanis-blobs-${process.pid}-${randomUUID().slice(0, 8)}`);
  ...
  async put(source: Readable | Buffer | string, meta: { contentType: string; filename?: string }): Promise<BlobHandle> {
    const id = randomUUID();
    let size = 0;
    const counted = Readable.from(source instanceof Readable ? source : [typeof source === 'string' ? Buffer.from(source) : source]);
    counted.on('data', (c: Buffer | string) => { size += typeof c === 'string' ? Buffer.byteLength(c) : c.length; });
    await pipeline(counted, createWriteStream(join(this.dir, id)));
    const handle: BlobHandle = { id, contentType: meta.contentType, size, ...(meta.filename ? { filename: meta.filename } : {}) };
  ...
  scope(): BlobScope {
    ...
    const parent = this;
    return {
      async put(source, meta) { const h = await parent.put(source, meta); mine.push(h); return h; },
      open: h => parent.open(h),
      drop: h => parent.drop(h),
      scope: () => parent.scope(),
      async release() { for (const h of mine.splice(0)) await parent.drop(h); },
    };
  }

  /** Remove the directory and everything in it. */
  destroy(): void { this.held.clear(); rmSync(this.dir, { recursive: true, force: true }); }
}
