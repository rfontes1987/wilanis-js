 ...
 */
import { expr, generate, rng, typeAt, type Type } from '@wilanis/core';
...
function narrow(a: Domain, b: Domain): Maybe<Domain> {
  ...
  if (b.absent) { if (d.present || d.eq !== undefined || d.truthy || d.minLen !== undefined) return UNSAT; d.absent = true; }
  if (b.present) { if (d.absent) return UNSAT; d.present = true; }
  if (b.truthy !== undefined) {
    if (d.absent) return UNSAT;
    if (d.truthy !== undefined && d.truthy !== b.truthy) return UNSAT;
    d.truthy = b.truthy; d.present = true;
  }
  // `ne` does not imply presence: a missing path satisfies 'x != lit' too, so absence stays compatible
  if (b.ne) d.ne = [...(d.ne ?? []), ...b.ne];
  for (const k of ['gt', 'gte', 'minLen'] as const) if (b[k] !== undefined) { if (d.absent) return UNSAT; d[k] = d[k] === undefined ? b[k] : Math.max(d[k]!, b[k]!); }
  for (const k of ['lt', 'lte', 'maxLen'] as const) if (b[k] !== undefined) { if (d.absent) return UNSAT; d[k] = d[k] === undefined ? b[k] : Math.min(d[k]!, b[k]!); }
  if (b.eq !== undefined) {
    if (d.absent) return UNSAT;
...
function solve(e: expr.Expr, want: boolean): Demands[] {
  switch (e.t) {
    case 'lit': return Boolean(e.v) === want ? [{}] : [];
    case 'has': return [want ? { [key(e.p)]: { present: true } } : { [key(e.p)]: { absent: true } }];
    case 'not': return solve(e.e, !want);
    case 'path': return [{ [key(e.p)]: { truthy: want } }];
    case 'len': {
    ...
    case 'bin': {
      if (e.op === '&&' || e.op === '||') {
        ...
        const ls = solve(e.l, want), rs = solve(e.r, want);
        if (!both) return [...ls, ...rs];
        const out: Demands[] = [];
        for (const l of ls) for (const r of rs) { const m = merge(l, r); if (m !== UNSAT) out.push(m); }
        return out;
      }
...
function compare(e: expr.Expr & { t: 'bin' }, want: boolean): Demands[] {
  ...
  const lit = (sides[li] as expr.Expr & { t: 'lit' }).v;
  ...
  if (target.t === 'len') {
    const inner = target.e;
    if (inner.t !== 'path' || typeof lit !== 'number') return [];
    return [{ [key(inner.p)]: lenDomain(truth, lit) }];
  }
  if (target.t !== 'path') return [];
  return [{ [key(target.p)]: valueDomain(truth, lit) }];
}
...
function valueDomain(op: Cmp, lit: unknown): Domain {
  ...
  switch (op) {
    case '>': return { gt: lit, present: true };
    case '>=': return { gte: lit, present: true };
    case '<': return { lt: lit, present: true };
    case '<=': return { lte: lit, present: true };
  }
}

function lenDomain(op: Cmp, n: number): Domain {
  switch (op) {
    case '==': return { minLen: n, maxLen: n, present: true };
    case '!=': return { minLen: n + 1, present: true }; // any length but n; longer is the simplest
    case '>': return { minLen: n + 1, present: true };
    case '>=': return { minLen: n, present: true };
    case '<': return { maxLen: n - 1, present: true };
    case '<=': return { maxLen: n, present: true };
  }
}
...
function nameable(e: expr.Expr): boolean {
  switch (e.t) {
    ...
    case 'has': case 'path': return true;
    case 'not': return nameable(e.e);
    ...
    case 'bin': {
      ...
      const t = sides[sides.findIndex(s => s.t === 'lit') === 0 ? 1 : 0];
      return t.t === 'path' || (t.t === 'len' && t.e.t === 'path');
    }
  }
}
...
export function branchesOf(rules: { when: string; to: string }[], elseTo: string): Branch[] {
  const out: Branch[] = [];
  const parsed = rules.map(r => { try { return expr.parse(r.when); } catch { return undefined; } });
  ...
  const allFalseBefore = (upto: number): Demands[] => {
    ...
    for (let j = 0; j < upto; j++) {
      ...
      const next: Demands[] = [];
      for (const a of alts) for (const f of solve(e, false)) { const m = merge(a, f); if (m !== UNSAT) next.push(m); }
      alts = next;
  ...
  for (let i = 0; i < rules.length; i++) {
    ...
    if (!e) { out.push({ ...base, demands: {}, unsolved: 'the expression does not parse' }); continue; }
    const mine = solve(e, true);
    const before = allFalseBefore(i);
    let hit: Demands | undefined;
    for (const m of mine) { for (const b of before) { const j = merge(m, b); if (j !== UNSAT) { hit = j; break; } } if (hit) break; }
    if (hit) { out.push({ ...base, demands: hit }); continue; }
...
export function satisfy(d: Domain, generated: unknown, t?: Type, seed = 1): unknown {
  ...
  if (d.ne?.length) {
    ...
    const plausible = (v: unknown) => typeof v !== 'number'
      ? true
...
function elementOf(t: Type | undefined, seed: number): unknown {
  if (t && t.kind === 'list') { try { return generate(t.of, rng(seed)); } catch { return null; } }
  return null;
}
...
export function setPath(root: unknown, path: string[], value: unknown): unknown {
  ...
  const base: Record<string, unknown> = root && typeof root === 'object' && !Array.isArray(root) ? { ...(root as Record<string, unknown>) } : {};
  if (!rest.length) {
    if (value === undefined) { delete base[head]; return base; }
    base[head] = value;
    return base;
  }
...
export function casesFor(
  ...
  return branches.map(branch => {
    ...
    return { at: found.at, branch, stubs, ...(input.length ? { input } : {}), ...(unreachable.length ? { unreachable } : {}) };
  });
}
...
function typeAtPath(t: Type | undefined, path: string[]): Type | undefined {
  ...
  try { const r = typeAt(t, path); return typeof r === 'string' ? undefined : r.type; } catch { return undefined; }
}
