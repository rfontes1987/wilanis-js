...
import { loadTree, type LoadResult, type PluginModule, type Refusal } from '@wilanis/core';
...
/** Load the tree at root with its plugins resolved: builtins, `extra`, and the packages project.json names. */
export async function loadProject(root: string, opts: { plugins?: Record<string, PluginModule> } = {}): Promise<LoadResult> {
  const { available, refusals } = await resolvePlugins(root, opts.plugins);
  const load = loadTree(root, available);
