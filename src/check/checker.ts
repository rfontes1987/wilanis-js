...
import { Scope, type OpHit } from './scope.js';
import { assignable, conforms, EMPTY_OBJECT, hasVars, show, STRING, substitute, typeAt, TypeError_, type Read, type Type } from '../types/type.js';
import * as expr from '../expr/expr.js';
import type { PluginModule } from '../plugins/plugin.js';

const RESERVED = new Set(['in', 'const', 'request', 'secrets']);

export interface PluginCheckContext { scope: Scope; refuse: (code: string, file: string, message: string, at?: string, hint?: string) => void; settings: Record<string, unknown> }
...
export function checkTree(load: LoadResult): RefusalList {
  ...
  for (const p of load.plugins) {
    ...
    (p as PluginModule & { check?: (c: PluginCheckContext) => void }).check?.({ scope, settings, refuse: (code, file, message, at, hint) => out.add({ code, file, message, at, hint }) });
  }
  return out;
}
