...
import { loadTree } from './loader/load.js';
import { checkTree } from './check/checker.js';
import { BUILTIN_PLUGINS } from './plugins/index.js';
import { describe, fuzz, ls, map, regress, rehearse, scaffold, summarize } from './tools/tools.js';
...
const USAGE = `wilanis -- declarative dataflow, judged by a compiler, run by a stateless kernel
  ...
  wilanis serve    [root] [--profile p]            start every trigger kind
  ...
  wilanis new      <kind> <name|path> [root] [--layer edge] [--port p] [--graph g] [--kind k]
...
function load(root: string) {
  ...
  return loadTree(abs, BUILTIN_PLUGINS);
}
...
async function main() {
  ...
  switch (cmd) {
    case 'check': { const l = check(rootArg(0)); console.log(`ok: ${l.registry.files.length} documents`); break; }
    case 'rehearse': {
      const l = check(rootArg(0));
    ...
    case 'fuzz': { const l = check(rootArg(0)); const w = await fuzz(l, { runs: flags.runs ? Number(flags.runs) : undefined, profile: flags.profile }); console.log(w.map(f => `wrote ${f}`).join('\n')); break; }
    case 'regress': { const l = check(rootArg(0)); const r = await regress(l, { profile: flags.profile }); console.log(r.lines.join('\n') || 'no scenarios -- run wilanis fuzz first'); if (!r.ok) process.exit(1); break; }
    case 'serve': {
      const l = check(rootArg(0));
    ...
    case 'run': {
      ...
      console.log(JSON.stringify(answer ?? report, null, 2));
    ...
    case 'ls': {
      ...
      console.log(ls(load(root), kind).join('\n'));
      break;
    }
    case 'describe': console.log(describe(load(rootArg(1)), positional[0])); break;
    case 'map': console.log(map(load(rootArg(0))).join('\n')); break;
    case 'new': {
