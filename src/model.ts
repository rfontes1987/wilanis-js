 ...
 * A document's kind is its $schema (@wilanis/<kind>.schema.json); its identity is its path (@...).
 */
...
export function kindOfSchema(s: unknown): Kind | undefined {
  ...
  const m = new RegExp(`^${WILANIS}/([a-z-]+)\\.schema\\.json$`).exec(s);
...
export interface ProjectDoc extends Envelope {
  ...
  plugins: { use: string; settings?: Record<string, unknown> }[];
