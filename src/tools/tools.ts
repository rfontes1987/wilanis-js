...
import { Scope } from '../check/scope.js';
import { Embedder } from '../triggers/embed.js';
import type { EffectInfo } from '../compiler/compiler.js';
import type { Handler, Report } from '../kernel/spec.js';
import { generate, rng, substitute, hasVars, show, type Type } from '../types/type.js';
import { schemaRef, splitOp, type Kind, type Loaded, type ScenarioDoc, type TriggerDoc, type TriggerKindDoc, type PortDoc, type GraphDoc, type AnyDoc } from '../model.js';
...
export async function regress(load: LoadResult, opts: { profile?: string } = {}): Promise<{ ok: boolean; lines: string[] }> {
  ...
  for (const sc of load.registry.all('scenario')) {
    ...
    if (trigger) report = await emb.fire(trigger.doc, sc.doc.in, sc.doc.request ?? {}, { stubs: sc.doc.stubs });
      ...
      const { runGraph } = await import('../compiler/compiler.js');
...
export function scaffold(root: string, kind: string, target: string, opts: Record<string, string | undefined>): string[] {
  ...
  const S = (k: Kind) => schemaRef(k);
  switch (kind) {
    case 'project':
      files.push(['project.json', { $schema: S('project'), name: target, description: 'TODO', aliases: {}, plugins: [{ use: '@std' }, { use: '@http', settings: { port: 8080, codecs: { 'application/json': '@http/codecs/json.codec.json' } } }], secrets: {} }]);
      break;
    case 'feature':
